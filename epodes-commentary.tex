...
\lem{Horrida}: The adjective \textit{horridus, -a, -um} often means `causing
...
the poem on a grim note.\indent\lem{caelum contraxit}: This phrase may have
...
\lem{imbres \lips nivesque deducunt Iovem}: A violent and surprising phrase.
...
\lem{Threico Aquilone} is scanned \begin{metrica}Thr\=e\-ic\-i\=o
...
\lem{Amici}: A famous textual crux.  The manuscripts universally read
...
\lem{virent genua}: The reference to knees may surprise us initially, but it
...
\lem{obducta solvatur fronte senectus}: Three spondees start this phrase,
...
accusative (\textit{pressa}).\indent\lem{tu}: If we keep \textit{amici} in line
...
line 3.\indent\lem{vina Torquato \lips consule pressa meo}: Wine jars would be
...
65.}\indent\lem{move} suggests that the addressee should dislodge the wine jar
from a shelf.
% -]] Let's drink while we're young 3-6
...
\lem{cetera}: Possibly a euphemism, modelled on a Greek usage.  In Greek it was
...
\textit{benigna} at line-end.}\indent\lem{mitte loqui}: The use of
...
38.3 \textit{mitte sectari}.}\indent\lem{deus}: In Homer, the narrator exhibits
...
situation.\indent\lem{benigna} is ablative with \textit{vice} on the next line
and not neuter with \textit{haec}.

\comment{8}{8}

\lem{reducet in sedem}: This phrase suggests putting something back in its
...
\lem{nunc} directs attention to the present moment, as opposed to the
...
right now.\indent\lem{Achaemenio}: Achaemenes was the (possibly mythical)
...
\lem{nardo \lips fide}: Perfume and music are expected at a Greco-Roman
drinking party.  \textit{fide} is from \textit{fidēs, fidis, f. - lyre} rather
than \textit{fidēs, fidēī, f. - trust, faith}.\indent\lem{Cyllenea} is scanned
...
\lem{fide \lips sollicitudinibus}: The ablative phrases are not parallel here.
...
The Centaurs were notoriously uncivilized and savage creatures, but Chiron was
...
\lem{nobilis \lips alumno}: If we set aside the \textit{ut}---which Horace
...
question with a reference to Thetis, Achilles' mother.}\indent\lem{cecinit}: In
...
\lem{invicte}: The initial placement of the vocative is emphatic and
...
end.\indent\lem{mortalis dea nate puer Thetide}: This appositive phrase expands
...
uses Achilles' name in the poem.\indent\lem{dea \lips Thetide}: Ablative of
...
\lem{manet} is ominous and threatening.\indent\lem{Assaraci tellus}: Another
...
\lem{quam \lips Simois}: The structure of this clause is as follows:
...
\lem{flavi}: Although the manuscripts universally offer \textit{parvi} here,
...
\lem{unde} must be taken closely with \textit{reditum} rather than with the
clause's verb \textit{rupere}.  It refers back to Troy.\indent\lem{certo
subtemine}: The \textit{subte(g)men} is the weft (or woof): transverse threads
...
\lem{rupere}: The perfect tense suits descriptions of fated events in Latin.
...
\textbf{dedi}} (1.279).\indent\lem{nec mater \lips}: This is not simply
...
home.\indent\lem{caerula}: Thetis, a water nymph, is of course the color of the
sea.

\comment{17}{17}

\lem{illic} points forward to Troy.  Horace's advice is for the present (see,
for example, \textit{nunc} in line 8), but Chiron's advice to Achilles is for
later---when Achilles is in Troy.\lem{omne malum vino cantuque levato}: The
...
the phrasing.\indent\lem{levato} is a second person singular future active
...
\lem{deformis} means ugly or unsightly.  Here it is used metonymically with
\textit{aegrimonia}, the misery which causes people to become
ugly.\indent\lem{aegrimoniae} is a rare word for misery.  The root
\textit{aeger} means \textit{wretched} or \textit{miserable}, and the suffix
\textit{-monia} indicates a quality (or more rarely an
activity).\indent\lem{alloquiis}: \textit{alloquium} is another rare word, made
